  ...
  module ConnectionAdapters
    module OracleEnhanced
      module DatabaseStatements
        ...
        # When the primary key is a single column we fall back to
        # `INSERT INTO ... (<pk>) VALUES (DEFAULT)`, which Oracle accepts as a
        # column-list `INSERT` whose value is the column's `DEFAULT` expression. This
        # lets the database fill the column from its column default — typically a
        ...
        # Composite primary keys (Array) and unknown primary keys (nil) cannot be
        # expressed with this single-column form, so we preserve the original
        # `NotImplementedError` for them.
        def empty_insert_statement_value(primary_key = nil)
          if primary_key && !primary_key.is_a?(Array)
